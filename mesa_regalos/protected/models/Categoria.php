...
class Categoria extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('fechaDeCreacion, nombre', 'required'),
			array('activo, posicion', 'numerical', 'integerOnly'=>true),
			array('imagen, metas', 'length', 'max'=>128),
			array('nombre', 'length', 'max'=>45),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('idCategoria, activo, fechaDeCreacion, posicion, imagen, metas, nombre', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'productos' => array(self::HAS_MANY, 'Producto', 'fkCategoria'),
			'subcategorias' => array(self::HAS_MANY, 'Subcategoria', 'fkCategoria'),
		);
	}
